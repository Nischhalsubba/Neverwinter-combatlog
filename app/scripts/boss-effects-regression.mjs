...
assert.equal(blood.applications, 3, 'valid ownerless Blood Lust markers must still count as boss target-effect applications');
...
const combatEngine = readFileSync(new URL('../src/engine/combat-effects.js', import.meta.url), 'utf8');
assert.match(combatEngine, /findSupportEffect/);
assert.match(combatEngine, /enemy-mechanic/);
assert.match(combatEngine, /unclassified-enemy-effect/);
...
assert.match(intelligenceEngine, /amount\/baseAmount/);
assert.match(intelligenceEngine, /unexplainedAmplification/);
assert.match(intelligenceEngine, /publishUptime/);

const ui = readFileSync(new URL('../src/v7/boss-effects.js', import.meta.url), 'utf8');
assert.match(ui, /dataset\.view = 'debuffs'/);
assert.match(ui, />Team Debuffs</);
assert.match(ui, /What made the boss take more damage?/);
assert.match(ui, /Party damage only/);
assert.match(ui, /effect-intelligence-report/);
assert.match(ui, /workerRequest/);
assert.match(ui, /Who applied it/);
assert.match(ui, /Target uptime/);
assert.match(ui, /Damage check/);
assert.match(ui, /Unexplained damage windows/);
assert.match(ui, /\^\(boss\|encounter\)/);
assert.match(ui, /targetOnly: false/);
assert.doesNotMatch(ui, /analyzeCombatEffects/);
assert.doesNotMatch(ui, /analyzeBossEffects/);
assert.doesNotMatch(ui, /isTeamDamageSupportEffect/);
assert.doesNotMatch(ui, /raw-page/);
assert.doesNotMatch(ui, /selectFirstBoss/);
...
assert.match(css, /debuff-detail-columns/);
assert.match(css, /debuff-immune/);
