...
const standard = '26:04:15:00:00:00.000::Alice,P[1 Alice],Alice,P[1 Alice],Boss,C[9 Dragon_Boss],Arcane Strike,Power_Arcane,Physical,Critical|CombatAdvantage,1000,1000';
const parsed = parseLine(standard, 1);
assert.equal(parsed.ok, true);
assert.equal(parsed.row.damageType, 'Physical');
assert.equal(parsed.row.validDamage, true);
assert.equal(parsed.row.amount, 1000);

const nonCanonical = parseLine('26:04:15:00:00:00.500::Alice,P[1 Alice],Alice,P[1 Alice],Boss,C[9 Dragon_Boss],Arcane Echo,Power_Arcane,Arcane,,700,700', 2);
assert.equal(nonCanonical.ok, true);
assert.equal(nonCanonical.row.kind, 'damage');
assert.equal(nonCanonical.row.validDamage, false, 'reference totals publish Physical damage rows only');

const legacyComma = '26:04:15:00:00:01.000::Alice, The Brave,P[1 Alice],Summoned, Wolf,C[7 Pet_Wolf],Ancient, Dragon,C[9 Dragon_Boss],Power, With, Comma,Power_Comma,Physical,Critical,500,500';
const recovered = parseLine(legacyComma, 3);
...
assert.equal(parseLine(indexed, 4).ok, true);

const text = [
  standard,
  '26:04:15:00:00:02.000::Alice,P[1 Alice],Alice,P[1 Alice],Boss,C[9 Dragon_Boss],Arcane Strike,Power_Arcane,Physical,,500,500',
  '26:04:15:00:00:30.000::Alice,P[1 Alice],Alice,P[1 Alice],Boss,C[9 Dragon_Boss],Arcane Strike,Power_Arcane,Physical,Critical,250,250',
...
assert.ok(alice.combatDps > alice.dps, 'combat DPS should exclude separate idle encounter gaps');

const excluded = parseText([
  standard,
  '26:04:15:00:00:01.000::Alice,P[1 Alice],Alice,P[1 Alice],Boss,C[9 Dragon_Boss],Arcane Echo,Power_Arcane,Arcane,,9000,9000'
].join('\n'));
assert.equal(excluded.summary.damage, 1000);
assert.equal(excluded.summary.validDamageRows, 1);
assert.equal(excluded.summary.nonCanonicalDamageTypes.find(item => item.key === 'Arcane')?.value, 1);
...
const rollover = parseText([
  '26:04:30:23:59:59.000::Alice,P[1 Alice],Alice,P[1 Alice],Dragon,C[9 Dragon_Boss],Arcane Strike,Power_A,Physical,,100,100',
  '26:05:01:00:00:01.000::Alice,P[1 Alice],Alice,P[1 Alice],Dragon,C[9 Dragon_Boss],Arcane Strike,Power_A,Physical,,100,100'
].join('\n'));
...
const phasedBoss = parseText([
  ...
  '26:04:15:00:00:14.000::Alice,P[1 Alice],Alice,P[1 Alice],Dragon,C[9 Dragon_Boss],Arcane Strike,Power_A,Physical,,200,200'
].join('\n'));
...
const phasedAlice = phasedBoss.summary.players.find(player => player.ref === 'P[1 Alice]');
assert.equal(phasedAlice.combatTime, 14, 'merged boss/add/boss time must count as in-combat time');
assert.equal(phasedAlice.combatDps, 5300 / 14);
