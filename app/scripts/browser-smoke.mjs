...
async function assertNoOverflow(cdp, label) {
  const metrics = await cdp.eval(`(() => ({scroll:document.documentElement.scrollWidth,client:document.documentElement.clientWidth,body:document.body.scrollWidth,inner:innerWidth}))()`);
...
async function waitRoute(cdp, view, timeout = 20000) {
  await waitFor(() => cdp.eval(`(() => { const button=document.querySelector('#app-nav [data-view="${view}"]'); const root=document.getElementById('view-root'); if(!button?.classList.contains('is-active')||!root)return false; return !root.querySelector('[data-task-loading][aria-busy="true"]')&&root.textContent.trim().length>20; })()`), timeout, 120, `${view} route`);
}
...
const chrome = spawn(binary, ['--headless=new','--no-sandbox','--disable-gpu','--disable-dev-shm-usage','--remote-debugging-port=9222',`--user-data-dir=/tmp/strikeglass-browser-${process.pid}`,'about:blank'], { stdio: ['ignore','ignore','pipe'] });
let cdp = null;
...
try {
  ...
  const toggled = await cdp.eval(`(() => { const b=document.querySelector('.v6-widget-drawer [data-v6-toggle]'); if(!b)return false; const before=b.getAttribute('aria-checked'); b.click(); b.click(); return b.getAttribute('aria-checked')===before; })()`);
  ...
  const bossScope = await cdp.eval(`(() => { const s=document.getElementById('encounter-select'); const o=[...s.options].find(x=>x.value.startsWith('boss:')); if(!o)return ''; s.value=o.value; s.dispatchEvent(new Event('change',{bubbles:true})); return o.value; })()`);
  ...
  assert.equal(await cdp.eval("Boolean(document.querySelector('#view-root .verification-blocked'))"), false, 'Team Debuffs must finish its verified effect analysis for the fixture');
  ...
  assert.match(await cdp.eval("document.querySelector('[data-qol-event-status]').textContent"), /Page 1/);
  if (await cdp.eval("Boolean(document.querySelector('[data-qol-continue-search]'))")) {
  ...
  await cdp.eval("document.querySelector('.sg-chart-studio')?.scrollIntoView({block:'center',inline:'nearest'})");
  await waitFor(() => cdp.eval(`(() => { const stage=document.querySelector('.sg-chart-studio [data-sg-chart-stage]'); const chart=stage&&window.echarts?.getInstanceByDom(stage); return Boolean(chart?.getOption()?.dataZoom?.[0]); })()`), 12000, 100, 'lazy Graph Studio instance');
  ...
  assert.ok(beforeZoom, 'Graph Studio must expose ECharts dataZoom state after entering the viewport');
  ...
  await cdp.eval(`(() => { const set=(name,value)=>{const s=document.querySelector('[data-setting="'+name+'"]');s.value=value;s.dispatchEvent(new Event('change',{bubbles:true}));}; set('theme','dark');set('contrast','high');set('density','comfortable');set('motion','reduced'); })()`);
  assert.deepEqual(await cdp.eval("({theme:document.documentElement.dataset.theme,contrast:document.documentElement.dataset.contrast,density:document.documentElement.dataset.density,motion:document.documentElement.dataset.motion})"), { theme:'dark',contrast:'high',density:'comfortable',motion:'reduced' });
  ...
  assert.ok(await cdp.eval("document.getElementById('workspace').getBoundingClientRect().width") > 1800, 'wide-screen workspace should actually use the available width');
  await screenshot(cdp, 'overview-2560');

  await setViewport(cdp, 390, 844, true);
  await assertNoOverflow(cdp, 'Overview at 390px');
  const mobileTargets = await cdp.eval(`(() => [...document.querySelectorAll('button:not([hidden]):not(:disabled)')].filter(b=>{const r=b.getBoundingClientRect();return r.width>0&&r.height>0&&r.top<innerHeight&&r.bottom>0;}).map(b=>({label:b.getAttribute('aria-label')||b.textContent.trim().slice(0,40),w:b.getBoundingClientRect().width,h:b.getBoundingClientRect().height})).filter(x=>x.w<40||x.h<40).slice(0,10))()`);
  ...
  if (cdp) {
    ...
    try { const snapshot=await cdp.eval(`(() => ({title:document.getElementById('workspace-title')?.textContent||'',view:document.querySelector('#app-nav [data-view].is-active')?.dataset.view||'',url:location.href,htmlTheme:{...document.documentElement.dataset},body:document.body.innerText.slice(0,12000)}))()`); await writeFile(join(artifactDir,'failure-state.json'),JSON.stringify(snapshot,null,2)); } catch {}
  }
  throw error;
} finally {
  chrome.kill('SIGTERM');
