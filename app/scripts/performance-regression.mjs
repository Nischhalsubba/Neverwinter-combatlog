...
const [app, worker, verifierWrapper, verifierCore, charts, motion, copy, styles, index, runtime, viewport, drilldown, dashboard] = await Promise.all([
  readFile('src/v3/app.js', 'utf8'),
  readFile('src/workers/fast-parse-worker.js', 'utf8'),
  readFile('src/engine/verification-engine.js', 'utf8'),
  readFile('src/engine/verification-engine-core.js', 'utf8'),
...
const verifier = `${verifierWrapper}\n${verifierCore}`;
...
for (const marker of ['seriesInOrder','orderedSeries','PROGRESS_ROWS','hashText','VERIFICATION_ENGINE_VERSION = 5']) assert.ok(verifierCore.includes(marker), 'preserved verifier core missing '+marker);
for (const marker of ['verifyArithmeticReport','classificationEvidence','verifyDirectRotationMarkers','VERIFICATION_ENGINE_VERSION']) assert.ok(verifierWrapper.includes(marker), 'verifier evidence wrapper missing '+marker);
assert.ok(!verifier.includes('Math.min(...partySeries.map'), 'verifier must not spread full session timelines');
assert.ok(!/function rotationChecksum[\s\S]*?flatMap/.test(verifierCore), 'rotation checksum must stream');
...
console.log('Performance regression passed. Route features load on demand, charts and motion have no CDN dependency, the preserved verifier core keeps ordered streaming checks, the evidence wrapper stays separate, mutation observers are removed from copy/debuff hot paths, cached route revisits skip skeletons, and Power Timing draws only the visible viewport.');
