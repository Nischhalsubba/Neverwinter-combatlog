...
const required=['index.html','src/v3/styles.css','src/v3/compact.css','src/v3/app.js','src/v3/motion.js','src/v3/ambient.js','src/engine/fast-parser-core.js','src/workers/fast-parse-worker.js'];
const failures=[];
for(const path of required){try{await access(path)}catch{failures.push(`Missing required V3 file: ${path}`)}}
const index=await readFile('index.html','utf8');
for(const marker of ['src/v3/styles.css','src/v3/compact.css','type="module" src="src/v3/app.js"','id="parse-state"','id="workspace"','id="drop-zone"'])if(!index.includes(marker))failures.push(`index missing ${marker}`);
...
for(const marker of ["new Worker(new URL('../workers/fast-parse-worker.js'","type: 'module'","request('player-report'","request('raw-page'",'timeline(points)','estimatedStoreBytes','const compact =','compactHtml','1e9','1e6','1e3'])if(!app.includes(marker))failures.push(`app missing ${marker}`);
...
const compactStyles=await readFile('src/v3/compact.css','utf8');
for(const marker of ['.empty-state','min-height:auto','.workspace-toolbar','.metric-card','.compact-number','@media(max-width:760px)'])if(!compactStyles.includes(marker))failures.push(`compact styles missing ${marker}`);
...
console.log('Smoke test passed. V3 worker architecture, compact data-first UI, parser diagnostics, Three.js budget, and GSAP motion contracts are present.');
