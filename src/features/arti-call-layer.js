(function(){
  ...
  function selectedDetails(result){
    ...
    return '<section class="arti-detail"><div class="arti-detail-head"><div><span class="eyebrow">Selected breakdown</span><h3>'+esc(selected.participant)+'</h3><p class="mut">These are only the damage numbers inside Arti Call windows for this '+esc(selected.sourceType.toLowerCase())+'. This is the drill-down, because apparently one table was not enough for endgame goblins.</p></div><div><b>'+fmt(selected.damage)+'</b><span>Total window damage</span></div></div>'+ 
      '<div class="grid2"><div><h3>Window-by-window stats</h3>'+table(rows,[['callId','Window #'],['artifact','Artifact used'],['caller','Callers'],['time','Started at'],['damage','Total damage'],['dps','Damage / sec'],['avgDamage','Avg damage'],['crit','Crit rate'],['flank','Flank rate'],['maxHit','Highest hit'],['maxPower','Highest-hit power']],300)+'</div><div><h3>Power breakdown</h3>'+table(selected.topPowers || [], [['power','Power'],['damage','Damage'],['hits','Hits'],['avgDamage','Avg'],['crit','Crit rate'],['flank','Flank rate'],['maxHit','Highest hit']],40)+'</div></div></section>';
  }
  ...
  function renderFromReport(result){
    ...
    content.innerHTML = playerHead + '<section class="panel arti-call-panel"><div class="arti-hero"><div><span class="eyebrow">Arti Call</span><h2>'+esc(result.windowSeconds || options.windowSeconds)+'-second damage inside artifact windows</h2><p class="mut">First artifact starts the timer. Any other artifact used before the timer ends joins that same window. The table below only shows what each player or companion did inside those windows. '+esc(modeNote)+'</p></div><div class="arti-window">'+esc(result.windowSeconds || options.windowSeconds)+'s<br><small>after first arti</small></div></div>'+controls()+
      '<div class="cards">'+
      ...
      '<div class="card"><b>'+fmt(totalParty)+'</b><span>Player + companion damage</span></div>'+ 
      '<div class="card"><b>'+num(participants.filter(row => row.sourceType === 'Player').length)+'</b><span>Players with damage</span></div>'+ 
      '<div class="card"><b>'+num(participants.filter(row => row.sourceType === 'Companion').length)+'</b><span>Companions with damage</span></div>'+ 
      '<div class="card"><b>'+esc(bestParticipant ? bestParticipant.participant : '-')+'</b><span>Top player / companion</span></div>'+ 
      '</div>'+ 
      '<h3>Damage by player and companion</h3><p class="mut">This is the main Arti Call answer: who did damage inside artifact windows, which artifact window they are tied to, and their combat quality stats.</p>'+participantTable(participants)+
      ...
      '<h3>By artifact</h3>'+table(result.byArtifact,[['artifact','Artifact / effect'],['windows','Windows'],['calls','Uses'],['users','Users'],['partyDamage','Party window damage'],['avgPartyDamage','Avg party damage'],['callerDamage','Caller damage'],['directDamage','Direct damage'],['bestUser','Best caller'],['bestCall','Best party window']])+ 
