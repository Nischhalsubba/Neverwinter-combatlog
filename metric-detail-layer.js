(function(){
  ...
  function openDrawer(label,value,context){
    ...
    d.querySelector('.metricDrawerBody').innerHTML='<span class="eyebrow">Metric breakdown</span><h2>'+esc(label||'Value')+'</h2><p class="drawerValue">'+esc(value||'')+'</p><section><h3>How it is calculated</h3><p>'+esc(inf.formula)+'</p><code>'+esc(inf.math)+'</code></section><section><h3>Current context</h3><dl><dt>Player</dt><dd>'+esc(s?.player||'-')+'</dd><dt>Scope</dt><dd>'+esc((window.state&&state.encounterId==='all')?'All encounters':'Selected encounter')+'</dd><dt>Table / card</dt><dd>'+esc(context||'-')+'</dd></dl></section><section><h3>Selected player totals</h3><dl><dt>Total damage</dt><dd>'+esc(s?fmt(s.total):'-')+'</dd><dt>DPS</dt><dd>'+esc(s?fmt(s.dps):'-')+'</dd><dt>Combat DPS</dt><dd>'+esc(s?fmt(s.combatDps):'-')+'</dd><dt>Hits</dt><dd>'+esc(s?s.hits.toLocaleString():'-')+'</dd></dl></section>';
